#!/usr/bin/env zsh
# check-docs.zsh — validate documentation cross-links and render-list
# reachability.
#
# Two passes:
...
#   2. Site reachability: every relative .md link in a file rendered by
#      Quarto must target another rendered file (per _quarto.yml's render
#      list), or it 404s on the published site.
#
...
local f l t
local -a files links
files=( README.md INTERNALS.md index.md(N) module/README.md(N) tests/README.md(N) )
for f in $files; do
